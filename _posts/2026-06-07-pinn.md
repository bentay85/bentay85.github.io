...
1. 1_Forward_Euler.py: Simulates the system using a state-space representation solved via the **Forward Euler** method. Compares numerical results directly against the analytical solution to check for accuracy. This script was written mainly to check how fast is a direct simulation method would be and for a simple 1D system like the mass-spring-damper system, it is definitely not worth it to use PINNs if the objective is just to simulate the position of the mass with time.  
